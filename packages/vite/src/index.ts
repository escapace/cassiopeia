...
const createPostPlugin = (state: State): Plugin => ({
  name: '@cassiopeia/vite:post',
  enforce: 'post',
  transform(source, id) {
    const query = parseVueRequest(id)

    if (
      state.files.has(query.filename) &&
      query.query.type === 'script' &&
      query.query.vue === true
    ) {
      ...
      // console.log(id, query.filename, map.has(query.filename))
