...
import { cassiopeia, Iterator, Plugin, renderToString } from './server'
...
describe('./src/server.spec.ts', () => {
  it('.', () => {
    ...
    const instance = cassiopeia({ id: 'id', plugins: [plugin] })
    assert.equal(state.i, 0)

    assert.equal(renderToString(instance, 'var(---abc-hello)'), undefined)
    assert.equal(state.i, 0)
